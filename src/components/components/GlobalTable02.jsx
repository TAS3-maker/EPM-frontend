...
const GlobalTable02 = ({
  ...
  const TruncateCell = ({ text }) => {
    return <div className="w-full truncate">{text || "—"}</div>;
  };
  ...
  const renderStandardRow = (item, index) => (
    ...
    >
      {columns.map((column) => (
        <td
          key={column.key}
          className="px-4 py-3 text-center text-gray-900 text-xs"
        >
          {column.render ? (
            column.render(item)
          ) : (
        ...
        >
          ...
          <td className="px-4 py-4 text-center">
            <input
              ...
              className={
                !hasActionableSheets ? "opacity-40 cursor-not-allowed" : ""
              }
...
"-"
  ...
  const renderModalRow = (sheet) => {
    ...
    const ApproveButton = ({ onClick }) => (
      <button
        onClick={onClick}
        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold shadow-lg shadow-green-600/30 transition-all"
      >
        ✓ Approve
      </button>
    );
    ...
    const RejectButton = ({ onClick }) => (
      <button
        onClick={onClick}
        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold shadow-lg shadow-red-600/30 transition-all"
      >
        ✕ Reject
      </button>
    );
        ...
        >
          ...
          {columns.map(({ label, key }, colIndex) => {
            ...
            return (
              <td
                key={key || colIndex}
                className={`px-4 py-4 ${label === "Project" ? "font-medium truncate" : label === "Time" ? "font-mono" : label === "Submitted on" ? "text-xs text-gray-500" : ""}`}
              >
                {content}
              </td>
            );
  ...
  const selectableRows = useMemo(() => {
    ...
    return (paginatedData || data).filter(
      (d) => !d?.sheets?.every((s) => s.status?.toLowerCase() === "rejected"),
    );
  ...
  const renderTableHeader = () => (
    ...
    >
      <tr
        className={
          tableType === "main"
            ? "table-th-tr-row table-bg-heading whitespace-nowrap sm:whitespace-normal"
            : "bg-white/60 backdrop-blur table-th-tr-row whitespace-nowrap sm:whitespace-normal"
        }
      >
