...
// }, []);
 ...
 useEffect(() => {
  try {
    ...
    if (pic) {
      // Try multiple domains until one works
      const urls = [
        `https://emp-staging.techarchsoftwares.com/api/storage/profile_pics/${pic}`,
        `https://epm.techarchsoftwares.com/api/storage/profile_pics/${pic}`,
        `http://13.60.180.240/api/storage/profile_pics/${pic}`
      ];
      
      // Test first working URL
      const testImage = new Image();
      testImage.onload = () => setUserimage(urls[0]);
      testImage.onerror = () => setUserimage(urls[1]); // fallback
      testImage.src = urls[0];
    }
  } catch (err) {
    console.error(err);
  }
