...
const ProjectCard = ({ project, expandedProjects, setExpandedProjects, openDeleteModal, navigate }) => {
    const maxVisible = 2;
    const teamLeads = project.team_leads || [];
    const isExpanded = expandedProjects?.[project.id] || false;
    const hasMore = teamLeads.length > maxVisible;

    // ✅ SAFETY: Never show projects without TLs
                ...
                <div className="py-4 px-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4 w-full">
                        ...
                        <div className="p-3 bg-gray-50 rounded-lg group-hover:bg-blue-50 transition-colors duration-300 flex flex-col items-center">
                            <div className="flex items-center mb-1">
                                ...
                                <span className="text-xs font-medium text-gray-600 ml-2">Remaining Hours</span>
                            </div>
                            <span className="text-sm font-bold text-gray-900">{project.remaining_hours || "N/A"}</span>
                        </div>
                        <div className="p-3 bg-gray-50 rounded-lg group-hover:bg-blue-50 transition-colors duration-300 flex flex-col items-center">
                            <div className="flex items-center mb-1">
                                <Briefcase className="w-4 sm:w-5 h-4 sm:h-5 text-blue-500" />
                    ...
                    <div className="pt-4 border-t border-gray-100">
                        <p className="font-semibold text-sm text-gray-800 mb-2 flex items-center">
                            ...
                            Assigned Team Leaders ({teamLeads.length})
                        </p>
                        <div className="flex flex-wrap gap-2 min-h-6">
                            {(isExpanded ? teamLeads : teamLeads.slice(0, maxVisible)).map((tl) => (
                                <span key={tl.id} className="inline-flex items-center bg-gray-100 rounded-full px-3 py-1 text-xs font-medium text-gray-700 shadow-sm">
                                    {tl.name}
                        ...
                        {hasMore && (
                            <button type="button"
                                ...
                                {isExpanded ? "Show Less" : `View All (${teamLeads.length})`}
                            </button>
                        )}
                    </div>
                </div>
...
export const PMassign = () => {
    ...
    // ✅ ULTRA-STRICT: ONLY projects WITH team_leads.length > 0
    const projectsData = useMemo(() => {
        if (!Array.isArray(employeeProjects?.data?.projects)) return [];
        
        return employeeProjects.data.projects
            .filter(project => project.team_leads && project.team_leads.length > 0)  // ✅ ONLY TL-assigned
            .map(projectWithTLs => {
                const assignedProject = assignedProjects.find(p => p.id === projectWithTLs.id);
                return {
                    ...assignedProject,
                    ...projectWithTLs,
                    team_leads: projectWithTLs.team_leads
                };
            });
...
const canAddEmployee=employeePermission==="2"
    ...
    const filteredProjects = useMemo(() => {
        ...
        return projectsData.filter((project) => {
            switch (filterBy) {
                ...
                case "team_leads": return project.team_leads?.some((tl) => tl?.name?.toLowerCase().includes(lowerCaseQuery));
    ...
    const handleTeamLeaderSelectionChange = (teamLeaderId) => {
        setSelectedTeamLeaders((prev) =>
            prev.includes(teamLeaderId) ? prev.filter((id) => id !== teamLeaderId) : [...prev, teamLeaderId]
        );
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!selectedProject || selectedTeamLeaders.length === 0) return;
        
        await assignProjectToTl(selectedProject, selectedTeamLeaders);
    ...
    const filteredAssignTeamLeaders = useMemo(() => 
        teamleaders.filter(tl => tl.name?.toLowerCase().includes(teamLeaderSearchQuery.toLowerCase())),
        [teamleaders, teamLeaderSearchQuery]
    );
    ...
    return (
        <div className="bg-gradient-to-b from-gray-50 to-white min-h-screen">
            <SectionHeader icon={BriefcaseBusiness} title="Projects Assigned" subtitle="Manage and track your assigned projects & team leaders" />
            ...
            <div className="max-w-full mx-auto p-2 sm:p-4">
                <div className="flex flex-wrap md:flex-nowrap items-center gap-3 border p-4 rounded-xl shadow-md bg-white mb-4 sm:mb-8">
                    <div className="relative flex items-center w-full flex-grow border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-blue-500 shadow-sm">
                        ...
                        {searchQuery && (
                            <button onClick={handleClearSearch}
                                className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-gray-500 rounded-full hover:bg-gray-100 hover:text-gray-700">
                    ...
                    <button onClick={() => setIsModalOpen(true)}
                        ...
                        <User className="h-5 w-5" /> Assign Team Leaders
                    </button>
                    )}
                </div>
                    ...
                    {isLoading || !employeeProjects?.data?.projects ? (
                        <div className="flex items-center justify-center min-h-[400px]">
                            <div className="flex items-center space-x-3 bg-white px-8 py-6 rounded-xl shadow-lg">
                                ...
                                <span className="text-lg text-gray-600">Loading projects with Team Leaders...</span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-center min-h-[400px]">
                            <div className="text-center bg-white p-8 rounded-xl shadow-lg">
                                <p className="text-xl font-semibold text-gray-700 mb-2">No projects with assigned Team Leaders</p>
                                <p className="text-gray-500">Assign Team Leaders using the button above</p>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg relative">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="relative" ref={projectDropdownRef}>
                                <label className="block font-semibold text-gray-700 mb-2">Project</label>
                                ...
                                {isProjectDropdownOpen && projectSearchQuery && (
                                    <div className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto">
                                        {filteredAssignProjects.map((project) => (
                                            <div key={project.id} className="cursor-pointer px-4 py-3 hover:bg-blue-50"
                                ...
                                {selectedProject && (
                                    <div className="mt-4 flex flex-wrap gap-2">
                                        <span className="flex items-center bg-blue-100 text-blue-800 text-sm font-medium px-4 py-2 rounded-full">
                                            ...
                                            <button type="button" onClick={() => { setSelectedProject(""); setProjectSearchQuery(""); }}
                                                className="ml-2 text-blue-600 hover:text-red-600">×</button>
                                        </span>
                                    </div>
                                )}
                            </div>

                            <div className="relative" ref={teamLeaderDropdownRef}>
                                <label className="block font-semibold text-gray-700 mb-2">Team Leaders</label>
                                <input type="text" placeholder="Search team leaders..." 
                                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                    value={teamLeaderSearchQuery}
                                    onChange={(e) => { setTeamLeaderSearchQuery(e.target.value); setIsTeamLeaderDropdownOpen(true); }}
                                    onFocus={() => setIsTeamLeaderDropdownOpen(true)} />
                                {isTeamLeaderDropdownOpen && teamLeaderSearchQuery && (
                                    <div className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto">
                                        {filteredAssignTeamLeaders.map((tl) => (
                                            <div key={tl.id} className={`cursor-pointer px-4 py-3 hover:bg-blue-50 flex items-center ${
                                                selectedTeamLeaders.includes(tl.id) ? "bg-blue-100 text-blue-800" : "text-gray-800"
                                            }`}
                                                onClick={() => { handleTeamLeaderSelectionChange(tl.id); setTeamLeaderSearchQuery(""); }}>
                                                {tl.name}
                                            </div>
                                        ))}
                                    </div>
                                ...
                                {selectedTeamLeaders.length > 0 && (
                                    <div className="mt-4 flex flex-wrap gap-2">
                                        {selectedTeamLeaders.map((id) => {
                                            ...
                                            return (
                                                <span key={id} className="flex items-center bg-green-100 text-green-800 text-sm font-medium px-4 py-2 rounded-full">
                                                    {tl?.name}
                                                    <button type="button" onClick={() => handleTeamLeaderSelectionChange(id)}
                                                        className="ml-2 text-green-600 hover:text-red-600">×</button>
                                                </span>
                                            );
                                        })}
                                    </div>
                            ...
                            <button type="submit" disabled={isAssigning || !selectedProject || selectedTeamLeaders.length === 0}
            ...
            {modalData && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
                    <div className="relative bg-white rounded-lg shadow-xl p-6 max-w-sm w-full">
                        <button onClick={closeDeleteModal} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
                            <X className="w-5 h-5" />
                        </button>
                        <h3 className="text-xl font-semibold text-gray-900 mb-4">Confirm Removal</h3>
                        <p className="text-gray-700 mb-6">
                            Remove "<span className="font-medium text-blue-600">{modalData.teamLeaderName}</span>" 
                            from "<span className="font-medium text-blue-600">{modalData.projectName}</span>"?
                        </p>
                        <div className="flex justify-end gap-3">
                            ...
                            <button onClick={async () => {
                                await deleteTeamLeader(modalData.projectId, modalData.teamLeaderId);
                                await Promise.all([
                                    fetchAssignedProjects(),
                                    fetchEmployeeProjects()
                                ]);
