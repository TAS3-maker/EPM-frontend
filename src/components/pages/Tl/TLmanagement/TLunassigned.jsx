...
onClick={() => setSelectedClientType("Assigned")}    className={`flex-1 min-w-[140px] text-center px-4 py-2 rounded-md font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 transition ${
  ...
  >
    Not Assigned
       ...
       <div
  key={project.id}
  className="group bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 relative"
>
  {/* Gradient Top Border */}
          ...
          {(isExpanded ? employees : employees.slice(0, maxVisible)).map((employee) => (
            ...
            >
              ...
              <button
                ...
                onClick={() =>
                  openDeleteModal(project.id, employee.id, employee.name, project.project_name)
                }
