...
const totalHours = savedEntries.reduce((sum, entry) => {
  ...
  if (raw.length === 0) {
    ...
    // If user enters single digit, pad with 0 for hours
    value = `0${raw}:`;
  } else if (raw.length === 2) {
    // Two digits for hours
    const hours = parseInt(raw.slice(0, 2), 10);
    if (isNaN(hours) || hours >= 10) return;
    value = `${raw.slice(0, 2)}:`;
  } else if (raw.length === 3 || raw.length === 4) {
    // Add minutes
    const hours = parseInt(raw.slice(0, 2), 10);
    const minutes = parseInt(raw.slice(2), 10);
    if (isNaN(hours) || isNaN(minutes) || hours >= 10 || minutes >= 60) return;
    value = `${raw.slice(0, 2)}:${raw.slice(2)}`;
  } else if (raw.length > 4) {
    return; // Prevent input longer than HHMM
  }
