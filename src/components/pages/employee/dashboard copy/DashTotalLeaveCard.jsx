...
const DashTotalLeaveCard = () => {
  const { id } = useParams();
  const today = new Date().toISOString().split('T')[0];
  const [startDate, setStartDate] = useState(today); // ✅ Default today
  const [endDate, setEndDate] = useState(today);
  const [isDateFilterOpen, setIsDateFilterOpen] = useState(false);
  const [filterApplied, setFilterApplied] = useState(false); // ✅ Filter state

  const navigate = useNavigate();
  const userRole = localStorage.getItem("user_name");
  
  const { leaves, fetchLeaves, leavesLoading } = useLeave();
  const activityDateFilterRef = useRef(null);

  useEffect(() => {
    console.log('📡 Fetching leaves for current user');
    fetchLeaves();
  }, [fetchLeaves]);
  ...
  // ✅ SAME FILTER LOGIC AS TotalLeaveCard
  const filteredLeaves = useMemo(() => {
    ...
    return safeLeaves.filter(leave => {
      ...
      const isStartInRange = leaveStart >= startDate;
      const isEndInRange = leaveEnd <= endDate;
  ...
  return (
    <div className='col-span-full xl:col-span-6 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden transform transition-all duration-300 ease-in-out hover:shadow-2xl hover:border-blue-200'>
      <div className="bg-white rounded-2xl shadow-md border border-gray-100 transition-shadow hover:shadow-lg duration-300 w-full md:max-w-50% relative min-h-60 max-h-[500px]">
        
        {/* ✅ SAME BEAUTIFUL FILTER DESIGN */}
...
export default DashTotalLeaveCard;
