...
const DashTotalWorkingHoursCard = () => {
//   const { id } = useParams();
  ...
  useEffect(() => {
    if (userId) {
      fetchWorkingHours(); // No dates = ALL DATA
    }
    ...
    <div className="col-span-full xl:col-span-12 bg-white border border-gray-100 rounded-2xl shadow-xl transform transition-all duration-300 ease-in-out hover:shadow-2xl hover:border-blue-200">
      <div className="px-5 py-2 flex justify-between items-center bg-gradient-to-br from-indigo-600 to-blue-500 rounded-t-xl">
        ...
        <div className="relative" ref={activityDateFilterRef}>
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 bg-white/20 rounded-lg px-2 py-1 text-white text-xs hover:bg-white/30 transition-all disabled:opacity-50"
          >
            📅 {workingStartDate && workingEndDate
              ? `${workingStartDate} - ${workingEndDate}`
              : 'All Dates'
            }
            <ChevronDown className={`w-3 h-3 transition-transform ${isWorkingDateFilterOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {isWorkingDateFilterOpen && (
            <div className="absolute right-0 mt-2 w-72 bg-white p-4 rounded-xl shadow-xl z-50 border">
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-gray-600 font-medium mb-1 block">End Date</label>
                  <input
                    type="date"
                ...
                <div className="space-y-2 pt-2 border-t border-gray-100">
                  <button
                    ...
                    disabled={!workingStartDate || !workingEndDate || loading}
                    className="w-full bg-indigo-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full bg-gray-200 text-gray-700 py-2 rounded-lg text-sm font-medium hover:bg-gray-300 disabled:opacity-50 transition-all"
                  >
                    Clear Filter
                  </button>
                </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-sm p-5 pb-8">
          ...
          <HourCard title="In House Hours" value={hoursData.inHouse} percentage={hoursData.percentages.inHouse} loading={loading} color="purple" />
          <HourCard title="Leave Hours" value={hoursData.leave} percentage={hoursData.percentages.leave} loading={loading} color="orange" />
        </div>
      )}
    </div>
  );
...
export default DashTotalWorkingHoursCard;
