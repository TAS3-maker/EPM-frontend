...
useEffect(()=>{
...
},[searchQuery,currentPage,filterBy])
  const fetchUsers = async (page=1,perPage=10,filters={}) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/get-permissions-allusers?${params}`, {
        headers: {
      ...
      if (data.success) {
        setUsers(data.permissions_of_users.data || []);
        setPaginationMeta({current_page:data.permissions_of_users.current_page,
          last_page:data.permissions_of_users.last_page,
          total:data.permissions_of_users.total
    ...
    <div className="mx-auto">
      ...
      {/* TABLE */}
        ...
        <GlobalTable
          ...
          emptyStateTitle={!loading && users.length === 0 ? "No users found" : "No users available"}
