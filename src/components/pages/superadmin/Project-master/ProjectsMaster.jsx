...
// 🔥 NEW: Clear tracking search when toggling same source
useEffect(() => {
  if (formData.use_same_source) {
    setTrackingSourceSearch(sourceSearch);
  } else {
    setTrackingSourceSearch("");
    setTrackingSourceAccounts([]);
  }
}, [formData.use_same_source, sourceSearch]);
...
const toggleSameSource = () => {
  setFormData(prev => ({
    ...
    ...(prev.use_same_source ? { 
      tracking_source_id: "", 
      tracking_account_id: "" 
    } : {})
  }));
  
  // 🔥 CRITICAL: Clear ALL tracking states immediately
  if (!formData.use_same_source) {
    setTrackingSourceSearch("");
    setTrackingSourceAccounts([]);
    setIsTrackingSourceDropdownOpen(false);
