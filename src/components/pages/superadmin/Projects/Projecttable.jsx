...
const flattenedProjects = (projects || []).map(project => ({
  ...
  client_number: project.client?.client_number || '',
  client_type: project.client?.client_type || '',
   client_created_at: project.client?.created_at ? new Date(project.client.created_at).toLocaleDateString() : '',
  // Flatten tags_activities names array into a comma-separated string
  tags_activities: (project.tags_activities || [])
    .map(tag => tag.name)
    .join(', '),
}));
...
project1Status: editStatus.trim().toLowerCase(),
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-md max-h-screen overflow-y-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 sticky top-0 bg-white p-4 z-10 shadow-md">
        ...
        <div className="flex flex-wrap md:flex-nowrap items-center gap-3 border p-2 rounded-lg shadow-md bg-white">
          ...
          <ExportButton onClick={() => exportToExcel(flattenedProjects, "projects.xlsx")} />
 ...
 project.client?.status || "No Status"
  )}
</td>
               <td className="px-6 py-4 items-center text-center text-gray-800 font-medium text-sm">
  {editProjectId === project.id ? (
    ...
    project.project_status
      ? project.project_status.charAt(0).toUpperCase() + project.project_status.slice(1).toLowerCase()
      : "—"
  )}
</td>
