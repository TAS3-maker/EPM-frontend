...
const TotalLeaveCard = () => {
  const { id } = useParams();
  const [startDate, setStartDate] = useState(''); // ✅ Empty by default = ALL DATA
  ...
  const { fetchLeavesByUserId, userSpecificLeaves, leavesLoading } = useLeave();
  ...
  // ✅ FIXED FILTER LOGIC - Date string comparison
  ...
  const filteredLeaves = useMemo(() => {
    if (!filterApplied || safeLeaves.length === 0) return safeLeaves;
    ...
    return safeLeaves.filter(leave => {
      ...
      // ✅ CORRECT DATE COMPARISON - string comparison works for YYYY-MM-DD
      const isStartInRange = !startDate || leaveStart >= startDate;
      const isEndInRange = !endDate || leaveEnd <= endDate;
      
      console.log(`Leave ${leave.id}: ${leaveStart} >= ${startDate}? ${isStartInRange}, ${leaveEnd} <= ${endDate}? ${isEndInRange}`);
      
      return isStartInRange && isEndInRange;
    });
  }, [safeLeaves, startDate, endDate, filterApplied]);

  const counts = useMemo(() => {
    const leaveData = filteredLeaves;
    
    console.log('📊 Total Safe:', safeLeaves.length, 'Filtered:', filteredLeaves.length);
    
    const pendingCount = leaveData.filter(leave => 
      leave?.status?.toLowerCase() === "pending"
    ).length;
    
    const approvedCount = leaveData.filter(leave => 
      leave?.status?.toLowerCase() === "approved"
    ).length;
    
    const rejectedCount = leaveData.filter(leave => 
  ...
  // ✅ APPLY FILTER - Set flag instead of refetching
      ...
      <div className="px-5 py-2 flex items-center justify-between bg-gradient-to-br from-indigo-600 via-blue-600 to-blue-500 rounded-t-xl">
        ...
        <div className="relative" ref={activityDateFilterRef}>
          <button 
            ...
            className="flex items-center gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm border border-white/30 rounded-lg px-2 py-1 text-white/90 hover:text-white transition-all duration-200 group text-xs font-medium disabled:opacity-50"
          >
            ...
            <ChevronDown className={`w-3 h-3 transition-transform ${isDateFilterOpen ? 'rotate-180' : ''}`} />
          </button>
              ...
              <div className="space-y-3">
                ...
                <div className="pt-2 border-t border-gray-100 space-y-2">
                  <button 
                    ...
                    disabled={!startDate || !endDate || leavesLoading}
                  ...
                  <button 
                    ...
                    disabled={leavesLoading}
                    className="w-full py-2 px-4 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-all duration-200 text-sm font-medium disabled:opacity-50"
                  >
                    Clear Filter
                  </button>
                </div>
